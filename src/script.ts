...
type Student = Pick<Person, "name" | "age">;
type Must = Required<Person>

let a: number | string = 2;
let b: string = "2";
let c: boolean = true;
let d: (string | number | Person)[] = [];
let e: Person = {
  name: "Bob",
  age: 99,
  isSmart: true,
...
const cartContents = new Set<number>();

const getTMDBData = async (url: string) => {
  return (await axios.get(url)).data;
};

const createMovieTile = (movie: Movie) => {
  ...
  trailerButton.addEventListener("click", async () => {
    ...
    const trailer = trailersData.results.filter((trailer: any) => {
